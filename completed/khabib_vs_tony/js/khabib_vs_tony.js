...
describe("Sample tests", () => {
  ...
  it("clear sample test 5: Draw", () => {
    const joAndNate = "jab kick kick elbow elbow Jab kick punch takedown hammerfist HAMMERFIST";
    assert.strictEqual(khabibVsTony(joAndNate),'Draw');
  });
});
...
describe("Random tests", () => {
  ...
  function kataHelper() {
    let res = [];
    let words = Math.floor(Math.random() * 101) + 50;
    while (words > 0) {
      let str = khabib.concat(tony, joe, nate)[Math.floor(Math.random() * 12)];
      let rand1 = Math.floor(Math.random() * 3), rand2 = Math.floor(Math.random() * 5);
