...
def khabib_vs_tony(joe_and_nate)
  ...
  joe_and_nate.scan(/#{(KHABIB + TONY).join('!?|')}/i).each do |action|
    ...
    if action == 'submission' && habib_hp > tony_hp && tony_hp < 20
      ...
      return 'Khabib win by submition' if habib_hp - tony_hp > (position == 'standing' ? 10 : 5)
      ...
      return 'Tony win by KO' if habib_hp < (position == 'standing' ? 30 : 20) && ap == 10
      habib_hp -= ap
    elsif %w[jab overhand нammerfist].include?(action)
      return 'Khabib win by KO' if tony_hp < (position == 'standing' ? 20 : 30) && ap == 10
      tony_hp -= ap
    end
    return 'Tony win by TKO' if habib_hp == 0
    return 'Khabib win by TKO' if tony_hp == 0
  ...
  return 'Khabib win by decision' if habib_points > tony_points
